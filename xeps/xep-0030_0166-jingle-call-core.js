...
function xmppPostWebCallInviteFallback({
  ...
  if (typeof globalThis.postWebCallInviteForConversation === "function") {
    const url = globalThis.postWebCallInviteForConversation({
      conversationId,
      conversationType,
      screenShare,
      roomOverride,
...
function handleXmppJingleMessageAction(actionPayload, { peerJid = "", screenShareFallback = false } = {}) {
  ...
  if (action === "propose") {
    ...
    const cachedFeatures = xmppCachedCallFeaturesForPeer(peerFull || peer);
    const lowerPeerFull = (peerFull || "").toString().trim().toLowerCase();
    const enableAcceptCompat = cachedFeatures.size <= 0
      || lowerPeerFull.includes("/movim.")
      || lowerPeerFull.includes("/cheogram.");
    xmppCallSessionById.set(id, {
      ...
      sendAcceptCompat: enableAcceptCompat,
      state: "proposed",
      createdAt: Date.now(),
    ...
    if (session?.direction === "outgoing") {
      ...
      session.timeoutId = window.setTimeout(() => {
        ...
        xmppPostWebCallInviteFallback({
          ...
          conversationType: entry.conversationType || "dm",
          screenShare: Boolean(entry.screenShare || screenShareFallback),
          reason: "XMPP ringing timed out. Posting Web Call invite.",
          reasonTone: "error"
        });
      ...
      void (async () => {
        const initiated = await xmppSendJingleSessionInitiate(session.peerFullJid || peer, id, {
          ...
          onError: () => {
            xmppPostWebCallInviteFallback({
              ...
              reason: "XMPP session-initiate failed. Posting Web Call invite.",
              reasonTone: "error"
            });
          }
        });
        if (!initiated) {
          xmppPostWebCallInviteFallback({
            ...
            conversationType: session?.conversationType || "dm",
            screenShare: Boolean(session?.screenShare || screenShareFallback),
            reason: "XMPP session-initiate did not send. Posting Web Call invite.",
            reasonTone: "error"
          });
        }
      })();
...
async function launchNativeXmppConversationCall({ screenShare = false, allowWebFallback = false } = {}) {
  ...
  if (xmppPeerLikelyWebCallOnly(peerTargetJid || peerJid)) {
    if (allowWebFallback) {
      xmppPostWebCallInviteFallback({
        ...
        conversationType: conversation.type || "",
        screenShare: Boolean(screenShare),
        reason: "Movim web client detected. Posting Web Call invite instead of native call.",
        reasonTone: "info",
        allowUiFallback: false
      });
    } else {
      showToast("Movim web client detected. Native XMPP call skipped.", { tone: "info", duration: 2800 });
    }
    return false;
  }
  ...
  if (conversation.type === "dm" && peerJid && (!globalThis.startNativeXmppCallSession || typeof globalThis.startNativeXmppCallSession !== "function")) {
    const started = xmppStartOutgoingCallProposal({
      ...
      onNoResponse: () => {
        showToast("No XMPP call response. Posting Web Call invite.", { tone: "error", duration: 2800 });
        if (allowWebFallback) {
          xmppPostWebCallInviteFallback({
            conversationId: conversation.id || "",
            conversationType: conversation.type || "",
