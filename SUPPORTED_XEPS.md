...
| XEP-0334 | Message Processing Hints | Partial | Outbound DM/MUC/correction messages now include `urn:xmpp:hints` processing hints: encrypted OMEMO payloads send `no-store`/`no-permanent-store`/`no-copy`, while plaintext sends include `store`. |
