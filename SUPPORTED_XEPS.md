...
| XEP-0249 | Direct MUC Invitations | Partial | Parses incoming `jabber:x:conference` direct room invites in DMs (including `xmpp:room?join`-style JIDs and nested `<continue/>` payload compatibility), maps invited rooms into `XMPP Spaces`, and supports outbound DM invites via `/invitexmpp <room-jid> [| reason [| password [| continue [| thread-id]]]]`. |
