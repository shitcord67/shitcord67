...
| XEP-0156 | Discovering Alternative XMPP Connection Methods | Partial | Login/discovery flows query host-meta endpoints (local gateway first, optional browser fallback), now probing both `/.well-known/host-meta(.json)` and root `/host-meta(.json)` variants, and parse XML XRD + JSON JRD payloads for WebSocket alt-connections. |
