...
| XEP-0334 | Message Processing Hints | Partial | Outbound DM/MUC/correction/reaction messages now include `urn:xmpp:hints` processing hints: encrypted OMEMO payloads send `no-store`/`no-permanent-store`/`no-copy`, while plaintext sends include `store`; disco feature responses now advertise `urn:xmpp:hints`. |
