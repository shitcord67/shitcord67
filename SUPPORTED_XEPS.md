...
| XEP-0334 | Message Processing Hints | Partial | Outbound DM/MUC/correction/reaction messages include `urn:xmpp:hints` processing hints (OMEMO sends `no-store`/`no-permanent-store`/`no-copy`; plaintext sends `store`), disco feature responses advertise `urn:xmpp:hints`, and inbound hints are now parsed and surfaced on messages as a hint badge/metadata. |
