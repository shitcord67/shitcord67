...
| XEP-0333 | Chat Markers | Partial | DM sends now include `markable` hints, inbound markable DMs receive `received` markers plus immediate `displayed` markers when that DM is actively open/visible, incoming `displayed`/`acknowledged` markers upgrade outbound DM status to read, and local DM read updates publish `displayed` markers via shared marker-flow XEP logic with broader reference-id fallback compatibility. |
