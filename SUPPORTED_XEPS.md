...
| XEP-0166 | Jingle | Partial | DM call session scaffolding now sends/handles `session-initiate`, `session-accept`, `session-info`, `transport-info`, and `session-terminate`, with per-session peer-connection candidate apply/queue behavior; full media transport/session negotiation is still incomplete. |
