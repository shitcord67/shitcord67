...
| XEP-0231 | Bits of Binary | Partial | Inbound inline BoB payloads (`urn:xmpp:bob`) are parsed for attachment rendering (including `cid:` references from XHTML `<img>`, `<source>`, and `<object>` paths), unresolved `cid:` sticker/media references trigger BoB IQ fetch attempts to hydrate Movim-style stickers, and plain-text `cid:` / `xmpp:cid:` tokens (including percent-encoded forms) now resolve through the BoB cache when available. Outbound BoB publish is not implemented. |
| XEP-0249 | Direct MUC Invitations | Partial | Parses incoming `jabber:x:conference` direct room invites in DMs (including `xmpp:room?join`-style JIDs, `uri`/`href` invite attributes, URI-encoded invite passwords, and nested `<continue/>` payload compatibility), now with body-text fallback parsing for `xmpp:`/join-link invite snippets when explicit invite payloads are omitted (including password query hints); maps invited rooms into `XMPP Spaces`, and supports outbound DM invites via `/invitexmpp <room-jid> [| reason [| password [| continue [| thread-id]]]]`. |
| XEP-0156 | Discovering Alternative XMPP Connection Methods | Partial | Login/discovery flows query host-meta endpoints (local gateway first, optional browser fallback), now probing both `/.well-known/host-meta(.json)` and root `/host-meta(.json)` variants, and parse XML XRD + JSON JRD payloads for WebSocket alt-connections with tolerant relation-token matching (including mixed-case/link variants and `rels`/`type` hints), plus object-mapped `links` compatibility in JRD payloads (including string-entry aliases in mapped link arrays). |
...
| XEP-0334 | Message Processing Hints | Partial | Outbound DM/MUC/correction/reaction messages include `urn:xmpp:hints` processing hints (OMEMO sends `no-store`/`no-permanent-store`/`no-copy`/`no-permanent-copy`; plaintext sends `store`), disco feature responses advertise `urn:xmpp:hints`, inbound hints are parsed for base/correction/retraction/reaction flows and surfaced on messages as hint metadata/badges, and `no-store`/`no-permanent-store` hints now suppress persistent local storage for those messages. |
| XEP-0030 | Service Discovery | Partial | Responds to incoming `disco#info` queries with supported client feature set (including call/Jingle signaling plus chatstates/receipts), uses `disco#items` for upload and MUC room discovery workflows, and tolerates non-strict `max-file-size` value formats when parsing upload limits. Incoming disco reply routing, IQ request helper flow, call interop assessment, and call-feature evaluation are centralized in the XEP module, including tolerant feature-set normalization for interop scoring and clearer `missing-features` vs `discovery-failed` outcomes. |
...
| XEP-0503 (Draft) | Spaces | Partial | Maintains a local registry of XMPP Spaces mappings and room metadata, with slash-command flows for list/open/sync/discover/join and leave room mapping actions; full server-backed hierarchy support is still under development. |
